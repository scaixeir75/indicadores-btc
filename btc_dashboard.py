...
st.set_page_config(page_title="Indicadores", layout="wide", page_icon="📊")
# ─── Ícone para ecrã do telemóvel (via JS no frame pai) ───────────────────────
...
(function() {{
    ...
    var existing = doc.querySelector('link[rel="icon"], link[rel="shortcut icon"]');
    if (existing) {{ existing.href = dataUrl; }}
    else {{
        var l = doc.createElement('link');
        l.rel = 'icon'; l.type = 'image/png'; l.href = dataUrl;
    ...
    if (apple) {{ apple.href = dataUrl; }}
        ...
        a.rel = 'apple-touch-icon'; a.href = dataUrl;
...
<style>
    ...
    .block-container { padding-top: 0.5rem; padding-bottom: 0.5rem; background-color: #131722; }
    h1, h2, h3, p, span, label { color: #d1d4dc !important; }
    [data-testid="stMetricLabel"], [data-testid="stMetricValue"],
    [data-testid="stMetricDelta"] { color: #b2b5be !important; }
    [data-testid="stSidebar"] { min-width: 220px; }
    .stInfo, .stSuccess, .stWarning { background: #1e222d; border: 1px solid #2a2e39; }
    div[data-testid="stPlotlyChart"] > div { background-color: #131722 !important; }
